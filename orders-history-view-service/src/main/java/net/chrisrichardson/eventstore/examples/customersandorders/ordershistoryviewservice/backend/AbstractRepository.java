...
public class AbstractRepository {
  private static final String DATABASE = "customers_and_orders";
  ...
  protected void findOneAndUpdate(String id, BasicDBObject newValue) {
    collection().findOneAndUpdate(new BasicDBObject("_id", id),
            new BasicDBObject("$set", newValue),
            upsertOptions());
  }

  protected Optional<Document> findOne(String id) {
  ...
  protected Money getMoney(Document document, String field) {
    BigDecimal bigDecimal = Optional
            .ofNullable(document.get(field, Decimal128.class))
